...
export class SaucesService {
  ...
  getSauces() {
    this.http.get('http://localhost:3001/api/sauces').subscribe(
      (sauces: Sauce[]) => {
  ...
  getSauceById(id: string) {
    return new Promise((resolve, reject) => {
      this.http.get('http://localhost:3001/api/sauces/' + id).subscribe(
        (sauce: Sauce) => {
          resolve(sauce);
        },
        (error) => {
  ...
  createSauce(sauce: Sauce, image: File) {
    return new Promise((resolve, reject) => {
      ...
      this.http.post('http://localhost:3001/api/sauces', formData).subscribe(
        (response: { message: string }) => {
          resolve(response);
        },
        (error) => {
  ...
  modifySauce(id: string, sauce: Sauce, image: string | File) {
    return new Promise((resolve, reject) => {
      if (typeof image === 'string') {
        this.http.put('http://localhost:3001/api/sauces/' + id, sauce).subscribe(
          (response: { message: string }) => {
            resolve(response);
          },
          (error) => {
        ...
        formData.append('sauce', JSON.stringify(sauce));
        formData.append('image', image);
        this.http.put('http://localhost:3001/api/sauces/' + id, formData).subscribe(
          (response: { message: string }) => {
            resolve(response);
          },
          (error) => {
  ...
  deleteSauce(id: string) {
    return new Promise((resolve, reject) => {
      this.http.delete('http://localhost:3001/api/sauces/' + id).subscribe(
        (response: { message: string }) => {
          resolve(response);
        },
        (error) => {
